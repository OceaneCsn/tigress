...
This package is an update of `tigress` to dela with RNASeq data non-normality, as pointed out in [this issue](https://github.com/jpvert/tigress/issues/6).
...
expr <- abiotic_stresses$raw_counts
dim(expr)
...
nstepsLARS = 5
tic()
edgepred <- tigress(t(ecoli$exp), tflist=tfnames, targetlist=targetnames, nstepsLARS = nstepsLARS, usemulticore = TRUE, nsplit = 50 )
toc()


nstepsLARS = 5
tic()
edgepredglm <- tigress(round(t(ecoli$exp), 0), tflist=tfnames, targetlist=targetnames, nstepsLARS = nstepsLARS, usemulticore = TRUE, nsplit = 50 , model = "glm")
toc()
```
